...
// --- пишущие ручки (заведение из кабинета) ---

/**
 * Завести партнёра в выбранном портале: контакт с полями партнёрки + сделка
 * в воронке партнёров на стадии «Сопровождение» (C2:WON), чтобы работал гейт доступа.
 * Основной портал пишет Диспетчер, Бирюзу пишет Бирюза.
 * @returns {{ contactId, dealId }}
 */
export async function createPartnerContact(payload, opts = {}) {
  const raw = await request('/partner', { method: 'POST', body: payload, ...opts });
  if (!raw?.contactId) throw new GatewayError('gateway не вернул contactId', { code: 'bad_response' });
  return { contactId: Number(raw.contactId), dealId: raw.dealId ? Number(raw.dealId) : null, existing: Boolean(raw.existing) };
}
