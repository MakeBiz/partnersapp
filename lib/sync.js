...
export async function upsertPartner(client, profile, { uplinePartnerId = null, portal = 'main' } = {}) {
  ...
  const { rows } = await client.query(
    `INSERT INTO partners
       (bitrix_contact_id, refcode, status, tier, points, comm_welcome, comm_base,
        welcome_used, welcome_deal_id, payer, pay_ratio, upline_partner_id, bitrix_portal)
     -- tier NOT NULL: gateway пока присылает null и перебил бы дефолт схемы
     VALUES ($1,$2,$3,COALESCE($4,'Сильвер'),0,$5,$6,COALESCE($7,false),$8,$9,$10,$11,$12)
    ...
    [
      ...
      profile.payer, profile.payRatio, uplinePartnerId, portal,
    ]
  );
  return rows[0];
}
...
export async function findPartnerByContact(client, contactId, portal = 'main') {
  const { rows } = await client.query(
    'SELECT * FROM partners WHERE bitrix_portal = $2 AND bitrix_contact_id = $1',
    [contactId, portal]
  );
  return rows[0] || null;
}
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO partners (bitrix_contact_id, refcode)
       VALUES ($1, $2)
       ON CONFLICT (bitrix_portal, bitrix_contact_id) DO UPDATE SET updated_at = now()
       RETURNING *`,
